...
export async function confirmSaveBeforeSubmit({
  title = "以下の内容で保存します。",
  lines = [],
  footer = "OKで保存します。キャンセルで入力に戻ります。"
} = {}) {
  ...
  const parts = [String(title || "").trim() || "以下の内容で保存します。", ""];
  if (normalized.length) {
    parts.push(...normalized);
    parts.push("");
  }
  parts.push(String(footer || "").trim() || "OKで保存します。キャンセルで入力に戻ります。");
